...
class MAx(Indicator):
    ...
    def calculate(self, idx):
        ...
        logger.info("self._pt._c[idx-self.smaPeriod:idx] value")
        logger.info(self._pt._c[idx-self.smaPeriod:idx])
        logger.info("self.smaPeriod value")
        logger.info(self.smaPeriod)
...
class BotTrader(object):
    ...
    def _botstate(self):
        ...
        if self.state != prev and self.state in [SHORT, LONG]:
            ...
            self.close()
            # self.order(units) #This code place order on platform

    def order(self, units):
        logger.info("order executed")
        logger.info("units value")
        logger.info(units)
        logger.info("instrument value")
        logger.info(self.pt.instrument)
        mop = {"instrument": self.pt.instrument,
               "units": units}

        def frmt(v):
            logger.info("frmt executed")
            logger.info("v value")
            logger.info(v)
